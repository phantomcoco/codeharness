...
final class MainViewModel: ObservableObject {
    @Published private(set) var state: PlanGateState = .idle
    ...
    private let controller: PlanGateController
    ...
    func selectWorkspace() {
        PlanGateTrace.log("ui.selectWorkspace.clicked")
    ...
    func loadDefaultWorkspace() {
        PlanGateTrace.log("ui.loadDefaultWorkspace.clicked")
        Task {
            do {
    ...
    func selectModel() {
        PlanGateTrace.log("ui.selectModel.clicked")
        let panel = NSOpenPanel()
    ...
    func loadDefaultModel() {
        PlanGateTrace.log("ui.loadDefaultModel.clicked")
        Task {
            do {
    ...
    func loadModel() {
        PlanGateTrace.log("ui.loadModel.clicked")
        Task {
            do {
    ...
    func unloadModel() {
        PlanGateTrace.log("ui.unloadModel.clicked")
        Task {
    ...
    func runSmokeTest() {
        PlanGateTrace.log("ui.smoke.clicked")
        Task {
            do {
    ...
    func loadSoulFile() {
        PlanGateTrace.log("ui.loadSoulFile.clicked")
        let panel = NSOpenPanel()
        panel.canChooseFiles = true
        panel.canChooseDirectories = false
        panel.allowsMultipleSelection = false
    ...
    func applyManualGuidance() {
        PlanGateTrace.log("ui.applyManualGuidance.clicked chars=\(manualGuidance.count)")
        Task {
    ...
    func clearSoulGuidance() {
        PlanGateTrace.log("ui.clearSoulGuidance.clicked")
        manualGuidance = ""
        Task {
    ...
    func generatePlan() {
        PlanGateTrace.log("ui.generatePlan.clicked taskChars=\(taskText.count)")
        Task {
    ...
    func approvePlan() {
        PlanGateTrace.log("ui.approvePlan.clicked")
        Task {
            do {
    ...
    func rejectPlan() {
        PlanGateTrace.log("ui.rejectPlan.clicked")
        Task {
    ...
    func executeApprovedPlan() {
        PlanGateTrace.log("ui.executeApprovedPlan.clicked")
        Task {
    ...
    func cancelCurrentOperation() {
        PlanGateTrace.log("ui.cancel.clicked")
        Task {
    ...
    private func refresh() async {
        PlanGateTrace.log("ui.refresh.start")
        ...
        PlanGateTrace.log("ui.refresh.done state=\(state) actions=\(actions.count) events=\(activity.count)")
    }

    private func setError(_ error: Error) async {
        PlanGateTrace.log("ui.error \(error)")
    ...
    private func projectRootURL() throws -> URL {
        ...
        while candidate.path != "/" {
            if FileManager.default.fileExists(atPath: candidate.appendingPathComponent("FixtureWorkspace", isDirectory: true).path),
               FileManager.default.fileExists(atPath: candidate.appendingPathComponent("PlanGate.xcodeproj", isDirectory: true).path) {
                return candidate
            }
        ...
        while url.path != "/" {
            if FileManager.default.fileExists(atPath: url.appendingPathComponent("FixtureWorkspace", isDirectory: true).path),
               FileManager.default.fileExists(atPath: url.appendingPathComponent("PlanGate.xcodeproj", isDirectory: true).path) {
                return url
            }
