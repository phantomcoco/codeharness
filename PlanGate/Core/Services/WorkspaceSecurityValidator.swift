...
public struct WorkspaceSecurityValidator: Sendable {
    ...
    public func canonicalWorkspace(from selected: URL) throws -> Workspace {
        ...
        PlanGateTrace.log("workspace.canonicalize selected=\(selected.path) canonical=\(canonical.path)")
    ...
    public func resolve(relativePath rawPath: String, in workspace: Workspace) throws -> URL {
        PlanGateTrace.log("workspace.resolve.start path=\(rawPath) root=\(workspace.canonicalRootURL.path)")
        ...
        PlanGateTrace.log("workspace.resolve.done path=\(rawPath) resolved=\(resolved.path)")
        return resolved
    }
