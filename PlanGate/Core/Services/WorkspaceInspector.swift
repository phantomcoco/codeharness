...
public struct WorkspaceInspector: WorkspaceInspecting {
    ...
    public func inspect(workspace: Workspace) async throws -> WorkspaceInspection {
        PlanGateTrace.log("workspace.inspect.start root=\(workspace.canonicalRootURL.path)")
        ...
        PlanGateTrace.log("workspace.inspect.done entries=\(entries.count) previews=\(previews.count) bytes=\(totalBytes)")
