...
public actor AllowlistedVerificationRunner: VerificationRunning {
    ...
    public func run(commandID: String, workspace: Workspace) async throws -> VerificationResult {
        PlanGateTrace.log("verify.run.start commandID=\(commandID) cwd=\(workspace.canonicalRootURL.path)")
        ...
        PlanGateTrace.log("verify.run.done commandID=\(commandID) exit=\(process.terminationStatus) duration=\(duration) outputChars=\(output.count)")
    ...
    public func cancel() async {
        PlanGateTrace.log("verify.cancel.request running=\(process != nil)")
        process?.terminate()
        process = nil
    }
