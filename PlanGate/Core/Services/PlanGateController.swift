...
public actor PlanGateController {
    public private(set) var state: PlanGateState = .idle
    public private(set) var workspace: Workspace?
    public private(set) var model: ModelConfiguration?
    public private(set) var guidance: BehaviorGuidance = .none
    public private(set) var task: PlanGateTask?
    ...
    private let stateMachine: PlanGateStateMachine
    ...
    public init(
        ...
        stateMachine: PlanGateStateMachine = PlanGateStateMachine(),
    ...
    public func selectWorkspace(_ workspace: Workspace) async {
        PlanGateTrace.log("controller.selectWorkspace.start path=\(workspace.canonicalRootURL.path)")
        ...
        PlanGateTrace.log("controller.selectWorkspace.done state=\(state)")
    }

    public func selectModel(_ model: ModelConfiguration) async {
        PlanGateTrace.log("controller.selectModel.start file=\(model.url.lastPathComponent)")
        await cancelCurrentOperation()
        self.model = model
        invalidateApprovalAndPlan("Model changed; approval invalidated.")
        state = .idle
        log(.model, "Model selected: \(model.url.lastPathComponent)")
        PlanGateTrace.log("controller.selectModel.done state=\(state)")
    }

    public func setGuidance(_ guidance: BehaviorGuidance) async {
        PlanGateTrace.log("controller.setGuidance.start source=\(guidance.source) chars=\(guidance.content.count)")
        await cancelCurrentOperation()
        self.guidance = guidance
        invalidateApprovalAndPlan("Behavior guidance changed; approval invalidated.")
        state = .idle
        PlanGateTrace.log("controller.setGuidance.done state=\(state)")
    }

    public func setTaskText(_ text: String) async {
        PlanGateTrace.log("controller.setTaskText chars=\(text.count)")
        task = PlanGateTask(text: text)
        invalidateApprovalAndPlan("Task changed; approval invalidated.")
    }

    public func loadModel() async throws {
        PlanGateTrace.log("controller.loadModel.start")
        ...
        PlanGateTrace.log("controller.loadModel.done inferenceState=\(loadedState)")
    }

    public func unloadModel() async {
        PlanGateTrace.log("controller.unloadModel.start")
        await inference.unloadModel()
        approval = nil
        log(.model, "Model unloaded; approval invalidated.")
        PlanGateTrace.log("controller.unloadModel.done")
    }

    public func generatePlan() async {
        PlanGateTrace.log("controller.generatePlan.start state=\(state)")
        do {
            ...
            let requestID = UUID()
            PlanGateTrace.log("controller.generatePlan.requestID=\(requestID)")
            ...
            PlanGateTrace.log("controller.generatePlan.inspection entries=\(inspection.entries.count) previews=\(inspection.filePreviews.count)")
            let messages = LlamaPromptBuilder.planningMessages(requestID: requestID, task: task, guidance: guidance, inspection: inspection)
            PlanGateTrace.log("controller.generatePlan.prompt chars=\(messages.reduce(0) { $0 + $1.content.count })")
            var output = ""
            ...
            PlanGateTrace.log("controller.generatePlan.modelOutput chars=\(output.count)")
            ...
            do {
                ...
                PlanGateTrace.log("controller.generatePlan.fallback reason=\(error) actions=\(fallback.actions.count)")
            ...
            if plan.actions != decodedPlan.actions {
                ...
                PlanGateTrace.log("controller.generatePlan.completedFixturePlan before=\(decodedPlan.actions.count) after=\(plan.actions.count)")
            }
            ...
            PlanGateTrace.log("controller.generatePlan.done planID=\(plan.id) actions=\(plan.actions.count)")
        } catch {
            PlanGateTrace.log("controller.generatePlan.failed error=\(error)")
            await fail(error)
        }
    }

    public func runNativeSmokeTest() async throws -> NativeSmokeTestResult {
        PlanGateTrace.log("controller.smoke.start")
        guard let model else { throw record(.modelNotSelected) }
        ...
        PlanGateTrace.log("controller.smoke.done completed=\(completed) tokens=\(tokenCount)")
        return result
    }
    ...
    public func approvePlan() throws {
        PlanGateTrace.log("controller.approvePlan.start state=\(state)")
        ...
        PlanGateTrace.log("controller.approvePlan.done planID=\(proposedPlan.id)")
    }

    public func rejectPlan() async {
        PlanGateTrace.log("controller.rejectPlan.start")
        approval = nil
        log(.approval, "Plan rejected; execution disabled.")
        do { try changeState(.idle) } catch { await fail(error) }
        PlanGateTrace.log("controller.rejectPlan.done state=\(state)")
    }

    public func executeApprovedPlan() async {
        PlanGateTrace.log("controller.executeApprovedPlan.start state=\(state)")
        do {
            guard let workspace else { throw AppFailure.workspaceNotSelected }
            ...
            PlanGateTrace.log("controller.executeApprovedPlan.authorized planID=\(plan.id) actions=\(plan.actions.count)")
            var writeResult: FileWriteResult?
            var verifyResult: VerificationResult?
            for action in plan.actions {
                PlanGateTrace.log("controller.executeApprovedPlan.action id=\(action.id) kind=\(action.kind)")
            ...
            PlanGateTrace.log("controller.executeApprovedPlan.done state=\(state)")
        } catch {
            PlanGateTrace.log("controller.executeApprovedPlan.failed error=\(error)")
            await fail(error)
        }
    }

    public func executeActMessage(_ text: String, requestID: UUID) async throws {
        PlanGateTrace.log("controller.executeActMessage.start requestID=\(requestID) chars=\(text.count)")
        ...
        case .writeFile(let path, let content):
            PlanGateTrace.log("controller.executeActMessage.write actionID=\(act.actionID) path=\(path) chars=\(content.count)")
        ...
        case .verify(let commandID):
            PlanGateTrace.log("controller.executeActMessage.verify actionID=\(act.actionID) commandID=\(commandID)")
        ...
        PlanGateTrace.log("controller.executeActMessage.done actionID=\(act.actionID)")
    }

    public func cancelCurrentOperation() async {
        PlanGateTrace.log("controller.cancel.start state=\(state)")
        activeTask?.cancel()
        ...
        PlanGateTrace.log("controller.cancel.done state=\(state)")
    }
    ...
    private func changeState(_ next: PlanGateState, hasApprovedPlan: Bool = false) throws {
        PlanGateTrace.log("controller.changeState.start current=\(state) next=\(next)")
        state = try stateMachine.transition(from: state, to: next, hasApprovedPlan: hasApprovedPlan)
        log(.state, "State -> \(state)")
        PlanGateTrace.log("controller.changeState.done state=\(state)")
    }

    private func invalidateApprovalAndPlan(_ reason: String) {
        PlanGateTrace.log("controller.invalidateApproval reason=\(reason)")
        approval = nil
        proposedPlan = nil
        log(.approval, reason)
    }
    ...
    @discardableResult private func record(_ failure: AppFailure) -> AppFailure {
        PlanGateTrace.log("controller.recordError failure=\(failure)")
    ...
    private func fail(_ error: Error) async {
        ...
        PlanGateTrace.log("controller.fail failure=\(failure)")
        log(.error, failure.description)
        state = .failed(failure)
    }

    private func log(_ kind: ActivityEvent.Kind, _ message: String) {
        PlanGateTrace.log("activity.append kind=\(kind) message=\(message)")
