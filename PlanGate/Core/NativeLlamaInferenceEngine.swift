...
actor NativeLlamaInferenceEngine: InferenceEngine {
    ...
    func loadModel(configuration: ModelConfiguration) async throws {
        PlanGateTrace.log("nativeLlama.loadModel.start file=\(configuration.url.lastPathComponent) bytes=\(configuration.fileSizeBytes)")
        ...
        if loadedModel == nil, modelParams.n_gpu_layers > 0 {
            PlanGateTrace.log("nativeLlama.loadModel.retryCPU after gpuLayers=\(modelParams.n_gpu_layers)")
            modelParams.n_gpu_layers = 0
        ...
        guard let loadedModel else {
            ...
            PlanGateTrace.log("nativeLlama.loadModel.failed reason=modelLoad")
        ...
        guard let loadedContext = llama_init_from_model(loadedModel, contextParams) else {
            ...
            PlanGateTrace.log("nativeLlama.loadModel.failed reason=contextInit")
        ...
        guard let loadedVocab = llama_model_get_vocab(loadedModel) else {
            ...
            llama_model_free(loadedModel)
            inferenceState = .failed(LlamaInferenceError.unsupportedModel.description)
            PlanGateTrace.log("nativeLlama.loadModel.failed reason=vocab")
            throw LlamaInferenceError.unsupportedModel
        }
        ...
        PlanGateTrace.log("nativeLlama.loadModel.done threads=\(configuration.threadCount) gpuLayers=\(effectiveGPULayerCount)")
    }

    nonisolated func generate(request: InferenceRequest) -> AsyncThrowingStream<InferenceEvent, Error> {
        PlanGateTrace.log("nativeLlama.generate.start requestID=\(request.id) maxTokens=\(request.maxTokens)")
        return AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    ...
                    PlanGateTrace.log("nativeLlama.generate.failed requestID=\(request.id) error=\(error)")
    ...
    nonisolated func cancelGeneration() async {
        PlanGateTrace.log("nativeLlama.cancelGeneration")
        cancellation.cancel()
    }

    func unloadModel() async {
        PlanGateTrace.log("nativeLlama.unloadModel.start hasModel=\(model != nil)")
        cancellation.cancel()
        ...
        PlanGateTrace.log("nativeLlama.unloadModel.done")
    }
    ...
    private func generate(
        ...
        PlanGateTrace.log("nativeLlama.generateText.start requestID=\(request.id) messages=\(request.messages.count)")
        ...
        PlanGateTrace.log("nativeLlama.generateText.tokenized tokens=\(promptTokens.count)")
        ...
        PlanGateTrace.log("nativeLlama.generateText.promptDecoded tokens=\(promptTokens.count)")
        ...
        PlanGateTrace.log("nativeLlama.generateText.done decodedTokens=\(decodedTokens) outputChars=\(output.count)")
